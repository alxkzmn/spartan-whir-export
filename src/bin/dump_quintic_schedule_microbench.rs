...
fn main() -> anyhow::Result<()> {
    ...
    anyhow::ensure!(
        args.len() == 1 || args.len() == 2 || args.len() == 7,
    ...
    let (num_variables, security_bits, merkle_bits, pow_bits, max_lir, output) = if args.len() == 7
    {
        (
            args[1].parse::<usize>()?,
            args[2].parse::<u32>()?,
    ...
    let dump =
        build_quintic_schedule_dump(num_variables, security_bits, merkle_bits, pow_bits, max_lir);
