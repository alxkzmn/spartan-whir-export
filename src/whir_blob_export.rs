...
use crate::{
    ...
    quintic_fixture::{RawWhirProof5, EF5},
...
const QUINTIC_EXTENSION_DEGREE: u8 = 5;
const QUINTIC_STATEMENT_POINTS: usize = 1;
const QUINTIC_STATEMENT_POINT_ARITY: usize = 22;
const QUINTIC_STATEMENT_EVALUATIONS: usize = 1;
const QUINTIC_INITIAL_OOD_SAMPLES: usize = 1;
const QUINTIC_ROUND_OOD_SAMPLES: usize = 1;
const QUINTIC_ROUND_COUNT: u8 = 3;
const QUINTIC_INITIAL_SUMCHECK_EVALS: usize = 8;
const QUINTIC_ROUND_SUMCHECK_EVALS: usize = 8;
const QUINTIC_FINAL_SUMCHECK_EVALS: usize = 12;
const QUINTIC_ROUND0_NUM_QUERIES: usize = 39;
...
pub fn encode_quintic_whir_k22_jb100_ext5_lir4_ff4_rsv4_blob_v1(
...
fn encode_quintic_whir_k22_jb100_ext5_lir4_ff4_blob_v1(
    ...
    final_queries: usize,
        ...
        statement_points.len() == QUINTIC_STATEMENT_POINTS,
        "expected exactly one statement point"
    );
    ensure!(
        statement_points[0].len() == QUINTIC_STATEMENT_POINT_ARITY,
        "expected statement point arity 22"
    );
    ensure!(
        statement_evaluations.len() == QUINTIC_STATEMENT_EVALUATIONS,
        "expected exactly one statement evaluation"
    );
    ensure!(
        raw_proof.initial_ood_answers.len() == QUINTIC_INITIAL_OOD_SAMPLES,
        "expected exactly one initial OOD answer"
    );
    ensure!(
        raw_proof.initial_sumcheck.polynomial_evaluations.len()
            == QUINTIC_INITIAL_SUMCHECK_EVALS / 2,
        "expected exactly four initial sumcheck rounds"
    );
    ensure!(
        raw_proof.rounds.len() == QUINTIC_ROUND_COUNT as usize,
        "expected exactly three non-final WHIR rounds"
    );
    ...
    let final_poly = raw_proof
        .final_poly
        .as_ref()
        .context("missing final polynomial in fixed quintic WHIR blob export")?;
    ensure!(
        final_poly.num_evals() == QUINTIC_FINAL_POLY_LEN,
        "expected final polynomial length 64"
    );
    ...
    let final_query_batch = raw_proof
        .final_query_batch
        .as_ref()
        .context("missing final query batch in fixed quintic WHIR blob export")?;
    let final_sumcheck = raw_proof
        .final_sumcheck
        .as_ref()
        .context("missing final sumcheck in fixed quintic WHIR blob export")?;
    ensure!(
        final_sumcheck.polynomial_evaluations.len() == QUINTIC_FINAL_SUMCHECK_EVALS / 2,
        "expected exactly six final sumcheck rounds"
    );
    ensure!(
        final_sumcheck.pow_witnesses.is_empty(),
    ...
    put_u8(&mut out, QUINTIC_EXTENSION_DEGREE);
    put_u8(&mut out, QUINTIC_ROUND_COUNT);
    put_u8(&mut out, FLAGS_V1);
    put_u16(
        &mut out,
        u16::try_from(round0_decomm_len).context("round0 decommitment count exceeds u16")?,
    ...
    encode_quintic_round(&mut out, round0, effective_digest_bytes, true);
    encode_quintic_round(&mut out, round1, effective_digest_bytes, false);
    encode_quintic_round(&mut out, round2, effective_digest_bytes, false);
...
fn encode_quintic_round(
    out: &mut Vec<u8>,
    round: &whir_p3::whir::proof::WhirRoundProof<F, EF5, u64, DIGEST_ELEMS>,
    effective_digest_bytes: usize,
    expect_base: bool,
) {
    put_digest(out, &round.commitment, effective_digest_bytes);
...
fn validate_quintic_round_query_batch(
    round: &whir_p3::whir::proof::WhirRoundProof<F, EF5, u64, DIGEST_ELEMS>,
    expected_queries: usize,
    expect_base: bool,
) -> anyhow::Result<usize> {
    ensure!(
        round.ood_answers.len() == QUINTIC_ROUND_OOD_SAMPLES,
        "round expected exactly one OOD answer"
    );
    ensure!(
        round.sumcheck.polynomial_evaluations.len() == QUINTIC_ROUND_SUMCHECK_EVALS / 2,
        "round expected exactly four sumcheck rounds"
    );
    let query_batch = round
        .query_batch
